...
def changeStdAttendance(user, student_id, lecture_id, isAttend):
    ...
    if (not lecture):
        return jsonify({'err': 'no such lecture'}), 404
    # std => should be his attendance and atetndance is open, valid secret
    if (user['role'] == 'std'):
        ...
        if (str(lecture.secret) != secret):
            print(secret)
            print(lecture.secret)
            return jsonify({'err': 'wrong secret'}), 400
        ...
        for student in studentsRoot.findall('Student'):
            if (student.find('StdCode').text == std['id']):
                ...
                root, error = API.getSchedule(
                    lectureInfo['schedule_id'], '9160830', '123')
                if(error):
                    return 'err', error, 500
...
def login():
    ...
    #return jsonify({'token': token, 'role': 'prof'}), 200

    token = jwtEncode({
        'username': "1142309", # رنا مصطفى عبدالمحسن محمد عبدالمولى schedule_id = 18218 (Machine Intelligence)
        'password': "-------",
        'role': 'std',
        'exp': datetime.utcnow() + timedelta(hours=48)
    }, app.config['SECRET_KEY']).decode('UTF-8')
    return jsonify({'token': token, 'role': 'std'}), 200
